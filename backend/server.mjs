...
const PORT = Number(process.env.KLE_API_PORT || 3001);
...
const server = createServer(async (req, res) => {
  ...
  try {
    if (req.method === 'POST' && url.pathname === '/api/auth/login') {
      ...
      const users = await readJson(usersFile);
      const found = users.find((item) => item.email.toLowerCase() === email && item.passwordHash === hashPassword(password));
      if (!found) {
    ...
    if (req.method === 'POST' && url.pathname === '/api/auth/register') {
      ...
      const sessions = await readJson(sessionsFile);
      const token = randomUUID();
      await writeJson(sessionsFile, [...sessions.filter((item) => item.email !== email), { token, email, createdAt: new Date().toISOString() }]);
    ...
    sendJson(res, 500, { error: 'Error interno del servidor.', detail: error instanceof Error ? error.message : String(error) });
  }
});

server.listen(PORT, () => {
  console.log(`KLE API listening on http://127.0.0.1:${PORT}`);
});
